...
void ABoidManager::Tick(float DeltaTime)
{
	...
	for (int i = 0; i < boids.Num(); i++)
	{
		...
		//{
		//	DrawDebugLine(GetWorld(), boids[i]->position, boids[i]->position + points[x] * collisionCheckDistance, FColor::Emerald, false, .1f);
		//}
		...
		if (IsGoingToCollide(i))
		{
			FVector collisionAvoidDir = FindAvoidDirection(i);
			FVector collisionAvoidForce = Move(collisionAvoidDir, i) * avoidWeight;
...
bool ABoidManager::IsGoingToCollide(int index)
{
	//hit result
	FHitResult hit;
...
FVector ABoidManager::FindAvoidDirection(int index)
{
